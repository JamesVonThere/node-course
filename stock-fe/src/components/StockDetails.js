...
const StockDetails = () => {
  ...
  useEffect(() => {
    let getPrices = (async () => {
      let response = axios.get('http://localhost:3001/stocks/2330')
    })
  }, [])
  return (
    <div>
      ...
      <div className="bg-white bg-gray-50 p-6 rounded-lg shadow m-6" key = {item.date} >
        ...
        <h2 className="text-2xl font-bold mb-2 text-gray-800">最高價：{item.transactions}</h2>
        <h2 className="text-2xl font-bold mb-2 text-gray-800">最低價：{item.volume}</h2>
        <h2 className="text-2xl font-bold mb-2 text-gray-800">成交筆數：{item.amount}</h2>
      </div>
          );
        })
      }
